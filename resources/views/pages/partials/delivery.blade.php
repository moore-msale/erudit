<div class="container delivery" style="">
    <div class="row">
        <div class="col">

        </div>
    </div>
</div>
<h2 class="font-weight-bold mb-3">Доставка и оплата</h2>
<p>
    2 способа получения заказов из интернет - магазина «Эрудит»: самовывоз, курьерская доставка.
</p>
<p>
    <span class="font-weight-bold">
    В связи со сложившейся ситуацией в стране, Тоговые Центры временно не работают.
    В данный момент принимаем заказы дистанционно, вы можете оставить заявку и с 11:00 до 15:00 часов
    мы отправим ваш заказ через курьерскую службу.
        </span>
{{--    <span class="font-weight-bold">Срок обработки заказа — один день.</span>--}}
</p>
<h3 class="font-weight-bold mt-5 mb-3">Оплата заказов</h3>
<p class="font-weight-bold">Оплата курьеру наличными.</p>
<p class="font-weight">Стоимость доставки в приделах города от 100 сомов и выше, в жилмассив 200 сом.</p>

<p>
    При получении вашего заказа курьер примет оплату наличными.
</p>
    <div class="row justify-content-center pt-4">
        <div class="col-12">
            <h2 class="font-weight-bold mb-3">Возврат</h2>
            <p>
                Купленный товар возврату не подлежит.
            </p>
            <p>Возврат осуществляется только в том случае, если товар окажется бракованным.</p>
            <p>Обмен купленного товара возможен, в случае предоставления чека.</p>
        </div>
    </div>
<div class="row justify-content-center mt-3">
    <div class="col-12">
        <h2>Контакты</h2>
    </div>
    <div class="col-12">
        <ul class="nav">
            <li class="nav-item">
                <a href="tel:+996 551 433 433" class="nav-link">
                    <i class="fas fa-phone"></i>&nbsp;+996 551 433 433
                </a>
            </li>
            <li class="nav-item">
                <a href="mailto:erudit.shop@mail.ru" class="nav-link">
                    <i class="fas fa-envelope"></i>&nbsp;erudit.shop@mail.ru
                </a>
            </li>
            <li class="nav-item">
                <a href="https://api.whatsapp.com/send?phone=996551433433" class="nav-link text-capitalize">
                    <i class="fab fa-whatsapp"></i>&nbsp;whatsapp
                </a>
            </li>
            <li class="nav-item">
                <a href="https://www.instagram.com/erudit_kg/?hl=ru" class="nav-link text-capitalize">
                    <i class="fab fa-instagram"></i>&nbsp;instagram
                </a>
            </li>
        </ul>
    </div>
</div>



