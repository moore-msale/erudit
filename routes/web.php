...
Route::get('/genre', 'Api\GenreController@index')->name('genre.all');

Route::get('/all_stock', function () {
    return view('pages.all_stock_page',['discounts' => \App\Book::where('discount','!=',null)->get()->reverse()]);
});

Route::get('/new_books', function () {
    return view('pages.new_books',['books' => \App\Book::all()->where('new',1)]);
});
Route::get('/promo_books', function () {
    return view('pages.promo_books',['books' => \App\Book::all()]);
});
Route::get('/comp_books', function () {
    return view('pages.comp_books',['compilation' => \App\Compilation::all()->first()]);
});
